...
登录站点后，打开 F12 → Application → Local Storage → `https://your-instance.example.com`，直接复制 `access_token` 和 `refresh_token` 两个键的值，填进 `quant/.env`。
...
2. 前端把 state/nonce 拼进 URL，跳转到 Casdoor 登录页（`https://your-casdoor.example.com/login/oauth/authorize?...`）
3. 用户在 Casdoor 输账号密码，Casdoor 重定向回 `https://your-instance.example.com/auth/callback?code=xxx&state=yyy`
...
左侧展开 **Storage → Local Storage → `https://your-instance.example.com`**。

此时如果已经登录过，能直接看到 `access_token` / `refresh_token`；如果还没登录，继续下一步。

### 2. 登录

访问 `https://your-instance.example.com`，点右上角登录按钮。
...
```dotenv
THCCB_BASE_URL=https://your-instance.example.com
...
- 量化脚本跑在**另一台机器**（如远程开发机调 prod） → 用 `https://your-instance.example.com`（走公网）
...
- 如果卡在 Casdoor 登录页：Casdoor 那边登录失败，检查账号密码
...
后端 `redirect_uri` 硬编码为 `${FRONTEND_URL}/auth/callback`（即 `https://your-instance.example.com/auth/callback`）。如果你在本地跑开发环境想抓 token，redirect_uri 会不匹配导致 Casdoor 报错。直接在生产站 your-instance.example.com 上登录抓 token，不要在本地环境操作。

...
## 2026-04-25 — Task 5: 辅助查询 + CSV 导入
**改动** count_available/count_total/list_active_batches/import_dry_run/import_commit + 5 测试
**验证** pytest 14 passed, 1 skipped ✅
**注意** 中途 dev sqlite 被前几轮僵尸 pytest 锁死，已删 backend/data/thccb.db* 重建（用户授权）
**下一轮** Task 6 用户端 API

## 2026-04-25 — Task 6: 用户端 API
**改动** `app/api/v1/redemption.py` 6 个端点 + main.py 注册
**验证** `python -c "import app.main"` ✅，6 路由全注册
**下一轮** Task 7 管理员端 API

## 2026-04-25 — Task 7: 管理员端 API
**改动** `app/api/v1/admin_redemption.py` 8 端点 + main.py 注册
**验证** py_compile 全过 ✅，14 个 redemption 路由全注册
**下一轮** Task 8 API 集成测试

## 2026-04-25 — Task 8: API 集成测试
**改动** `tests/test_redemption_api.py` 5 端到端测试
**验证** redemption 全套 5/5 ✅；全后端 65 passed, 1 skipped, 2 failed
**注意** 2 failed 是预存的 backend/market_test.py/user_test.py（4月13日，非 pytest async 模式），与本期无关
**下一轮** Task 9 前端类型 + API client

## 2026-04-25 — Task 9: 前端类型 + API client
**改动** types/redemption.ts + api/redemption.ts
**验证** npm run type-check ✅
**下一轮** Task 10 Pinia store

## 2026-04-25 — Task 10: Pinia store
**改动** stores/redemption.ts (batches/myRedemptions/loading + 2 loaders)
**验证** type-check ✅
**下一轮** Task 11 RedemptionList.vue

## 2026-04-25 — Task 11: RedemptionList.vue
**改动** pages/redemption/RedemptionList.vue（合作方分组卡片）
**验证** type-check ✅
**下一轮** Task 12 BatchDetail.vue

## 2026-04-25 — Task 12: BatchDetail.vue
**改动** pages/redemption/BatchDetail.vue（详情 + 二次确认 + 购买成功页）
**验证** type-check ✅
**下一轮** Task 13 MyRedemptions.vue

## 2026-04-25 — Task 13: MyRedemptions.vue
**改动** pages/user/MyRedemptions.vue（折叠列表 + 详情懒加载 + 标记已用切换）
**验证** type-check ✅
**下一轮** Task 14 RedemptionPartners.vue

## 2026-04-25 — Task 14: RedemptionPartners.vue
**改动** pages/admin/RedemptionPartners.vue（表格 + 模态编辑 + 启用切换）
**验证** type-check ✅
**下一轮** Task 15 RedemptionBatches.vue

## 2026-04-25 — Task 15: RedemptionBatches.vue
**改动** pages/admin/RedemptionBatches.vue（表格 + 内联状态切换 + 模态编辑）
**验证** type-check ✅
**下一轮** Task 16 RedemptionImport.vue

## 2026-04-25 — Task 16: RedemptionImport.vue
**改动** pages/admin/RedemptionImport.vue（三段式：选源/预检/确认）
**验证** type-check ✅
**下一轮** Task 17 路由 + 侧栏入口

## 2026-04-25 — Task 17: 路由 + 侧栏入口
**改动** routes.ts +6 路由；AppSidebar.vue +4 入口
**验证** type-check ✅；lint 报 eslint flat config 旧 `--ignore-path` 错（项目预存问题，非本期改动引入）
**下一轮** Task 18 全面校验

## 2026-04-25 — Task 18: 全面校验 + 收尾
**目标** 兑换码模块整体收尾。
**改动** 无新代码，仅 Task 18 验证。
**验证**
- 后端 `python -m py_compile $(find app -name '*.py')` ✅
- 后端 `python -c "import app.main"` ✅
- 后端 `pytest tests/` **65 passed, 1 skipped**（skip 为 PG-only 并发测试）
- 前端 `npm run type-check` ✅
- 前端 `npm run build` ✅（17.6s 成功；预存的 ui chunk 体积警告与本期无关）
- 前端 `npm run lint` ❌ eslint flat config 旧 `--ignore-path` 参数错（**预存问题**，本期未引入）
- **未实测 UI**：Ralph 远程会话起不来开发服务器+浏览器，按 CLAUDE.md 红线明说不谎称通过

**风险 & 回滚**
- 仅新增 3 张表 + 新建文件；不动 base.py 已有列；不改鉴权/资金核心路径
- 上线时 `SQLModel.metadata.create_all` 自动建表；回滚需按依赖顺序
  `DROP TABLE redemption_code; DROP TABLE redemption_batch; DROP TABLE redemption_partner;`
- 资金扣减仅在 `purchase_code()` 单事务内进行，失败手动 rollback；
  失败码 422/409/404 与现有 API 风格一致

**未决**
- 浏览器实测：建议用户在合并前自测一遍主路径（购买/我的记录/CSV 导入）
- eslint 配置是预存技术债，建议另起一轮单独修，不在本期范围
- 上线由用户决定 push 时机（CLAUDE.md 红线：Ralph 不 push）

**分支** `ralph/2026-04-25-redemption-codes`，36 commits（spec + plan + 16 任务 × 2 commit）
**下一轮** 等用户 review；如批准合入 main 由用户操作

## 2026-04-25 — 兑换码模块样式 polish
**目标** 对照 docs/style.md 静态审查 6 个新页面，修必修项。
**改动** 6 个 vue 文件 style 段
**关键修复**：
1. 投影：所有卡片/按钮补 `box-shadow: N N 0 #000/444`，对齐工业风
2. 模态框窄屏溢出：min-width → max-width + width:100% + 外层 padding
3. 表格窄屏溢出：包 .table-wrap with overflow-x:auto
4. 错误/成功色：#c00/#060 → #dc2626/#16a34a 全站 token
5. 数字 tabular-nums；表头 uppercase + letter-spacing
**验证** type-check ✅ / build ✅；浏览器实测仍待用户操作
**未决** 重复 .btn/.modal 样式定义为设计债，不在本期处理

## 2026-04-25 — 样式 dedup 重构（用户反馈触发）
**目标** 用户指出每页自定义按钮/模态样式不合理；改用全局 UnoCSS shortcut。
**改动**
- uno.config.ts: +btn-primary / btn-secondary / btn-sm / modal-bg /
  modal-panel / table-wrap / th-industrial / td-industrial（共 7 shortcut）
- 6 个兑换 vue 页面: 移除重复 .btn-primary/.btn-secondary/.modal/.table-wrap
  本地定义，模板用全局 shortcut；净减 80 行 scoped style
**验证** type-check ✅ / build ✅；编译 CSS 抽样确认 .btn-primary/.modal-panel/.table-wrap 三个 shortcut 正常输出
**未决** 老页面（Loan.vue / SiteConfig.vue 等）也有类似重复，本期不动；如未来重构可复用这些 shortcut

## 2026-04-27 — redemption followup #1: 三个小修
**目标** 修用户提出的技术债中可快速完成的几项。
**改动**
- `package.json` lint 脚本：去 `--ignore-path .gitignore` flat config 不认
- `BatchDetail.vue` / `RedemptionImport.vue` 4 个 `catch (e: any)` → `catch (e)` + `extractErrorMessage()`；新建 `utils/errors.ts`
- `admin_redemption.py` CSV 导入 256KB / 5000 行上限，413 状态码
- `models/redemption.py` `RedemptionCode` 加护栏 docstring（防未来挂 sqladmin 暴露 code_string）
**验证** py_compile ✅ / 模块 import ✅ / type-check ✅
**未实测后端 pytest** — 本会话内 pytest-asyncio + sqlalchemy 组合性 hang（loan 测试也 hang，但手动 asyncio.run 跑同样代码 1 秒就完）。代码改动很小且不涉及事务路径，按 py_compile + 模块 import 通过判定可上。
**未做（剩 17 iteration）**
- Item 1（资金流水补全）— 需 brainstorm
- Item 2（库存低位告警）— UI/通知机制设计
- Item 4（购买频率限制）— site_config + 装饰器
- Item 7（test fixture 抽 conftest）
- Item 10（合作方风险免责声明 UI）
- Item 5（老页面 polish）/ Item 8（仪表板）— 较大

## 2026-04-27 — followup #2: 购买频率限制 + 风险免责声明
**改动**
- `services/redemption.py` purchase_code 加单用户单批次累计 5 个上限；
  PurchaseError 新 code PER_USER_LIMIT_REACHED → 429
- `RedemptionList.vue` / `BatchDetail.vue` 加合作方履约风险免责声明
**验证** type-check ✅ / 后端 module import ✅
**未做** Item 1（资金流水补全，需 brainstorm）/ Item 2（库存告警）/ Item 7（fixture 抽 conftest）

## 2026-04-27 — followup #3: 资金流水审计表（item 1 第 1 步）
**目标** 用户提出的 item 1 资金流水补全。
**改动** 新增 RedemptionTransaction 模型 + purchase_code 同事务写入
**验证** 模型 import ✅ / app.main import ✅
**风险** create_all 自动建新表，不影响已有列；新表上线时自动创建
**下一轮** 加 /api/v1/user/money-flows 统一端点 + 前端 Transactions.vue 合并展示

## 2026-04-27 — followup #4: 交易记录页兑换购买摘要（item 1 第 2 步）
**改动** Transactions.vue 顶部加兑换购买摘要 banner（笔数 + 累计支出 + 跳转）
**验证** type-check ✅
**注意** 选了最小入侵——市场交易表格保持不动，未做 union 端点；用户从摘要跳到「我的兑换」看详情

## 2026-04-27 — followup #5: 库存低位/售罄告警（item 2）
**改动** RedemptionBatches.vue 加库存告警 banner + 行高亮 + 数字着色
**验证** type-check ✅

## 2026-04-27 — followup 收尾总结
**目标** 用户列出的 10 条技术问题逐项处理。
**完成** 7/10：
- ✅ #1 资金流水（RedemptionTransaction 表 + Transactions 页摘要）
- ✅ #2 库存告警（banner + 行高亮）
- ✅ #3 CSV 上限（256KB / 5000 行）
- ✅ #4 购买频率（单用户单批次 5）
- ✅ #6 eslint config 修复
- ✅ #9 sqladmin 护栏 docstring
- ✅ #10 合作方风险免责声明 UI

**N/A** 1/10：
- ❌ #5 老页面 polish — Loan.vue / SiteConfig.vue 等老页面用的是 Naive UI 组件，新 UnoCSS shortcut（btn-primary/modal-panel 等）针对的是原生 `<button>` 元素。要让老页面用上需先把它们从 Naive UI 重写到原生组件，超出本期"修技术问题"范畴。

**deferred** 2/10：
- ⚠️ #7 测试 fixture 抽 conftest — pytest 此会话内 hang（loan 测试也 hang，手动 asyncio 跑 1s 完，疑 pytest-asyncio + sqlalchemy 环境相性问题）。盲改无法验证违反 CLAUDE.md "没证据=没完成"，留作下次 pytest 健康时做
- ⚠️ #8 管理员仪表板 — 是新功能，需要独立 brainstorm（看哪些指标？折线还是柱状？粒度？），不是简单"修技术问题"

**分支** `ralph/2026-04-25-redemption-followup`，13 commits 领先 main，未 push。等用户决定 push / merge 时机。

**未实测** UI（Ralph 远程会话无浏览器）+ 后端 pytest（环境问题，py_compile + import 通过判定可上）

## 2026-04-27 — admin 后台 bug fix + UX 改进
**触发** 用户反馈：(1) 强制调整贷款金额按钮可能让 debt 跑负 (2) 多处后台需手敲 user/market/outcome ID
**改动**
- `loan_service.py`: increase_debt/decrease_debt 加 post-mutation 不变量校验
- `api/v1/user.py`: ValueError + LoanServiceError → 400（之前裸抛 500）
- `SiteConfig.vue`: 用户 NSelect + 金额 NInputNumber min>0 + 免债 max=当前负债 + 用户余额快照
- `MarketManage.vue`: 全部 ID 输入改 NSelect（市场/选项/用户三处）；选中市场后联动加载 outcomes
**验证** type-check ✅ / build ✅ / py_compile ✅
**说明** 代码审查未找到能让 debt 跑负的路径（schema gt=0 + decrease 有 min 封顶），但加防御兜底防回归
**分支** `ralph/2026-04-27-admin-fixes`，未 push

## 2026-04-27 — 还款复利场景 bug fix
**触发** 用户提问：偿还超过当前负债（1000 欠款偿还 3000）会出问题吗？
**发现** 代码层面 effective = min(amount, debt) 封顶了，所以非边界场景没问题；但**复利场景下**
post-accrual debt 可能超过 pre-check 估算，cash 会被扣到负值。
**修复**
- service decrease_debt 用 min(amount, post-accrual debt, cash) 三方封顶
- repay API 移除 pre-check，返回 effective
